...
async function initTrending (lang, since, type = "repositories") {
  ...
  try {
    ...
    console.log(body)
    let record = await mp.databaseoperate('databasequery', filterString(`
    db.collection("trending").where({
      lang: ${JSON.stringify(lang || "")},
      since: ${JSON.stringify(since || 'daily')},
      type: ${JSON.stringify(type || 'repositories')},
    ...
    if (data && data.length) {
      ...
      if (item && typeof item === 'string') {
        ...
        let res = await mp.databaseoperate('databaseupdate', filterString(`
    ...
    let res = await mp.databaseoperate('databaseadd', filterString(`

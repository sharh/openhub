		...
		if(inlineToken.type === 'htmlblock'){
			...
			while(match = videoRegExp.exec(html)){
				if (match[ 1 ]) {
					...
					var retParam = {
						type: 'video',
						src: isRelativeUrl(match[1]) ? baseUrl + match[1] : match[1]
					};

					if (match[ 3 ]) {
						if (/^\/\//.test(match[ 3 ])) {
							match[ 3 ] = 'https:' + match[ 3 ];
						}
						match[ 3 ] = (match[ 3 ] || '').replace(/^\/*/, '')
						retParam.poster = isRelativeUrl(match[3]) ? baseUrl + match[3] : match[3];
					}
			...
			inlineToken.children && inlineToken.children.forEach(function(token, index){
				if(['text', 'code'].indexOf(token.type) > -1){
					...
					token.src = (token.src || '').replace(/^\/*/, '')
					ret.push({
						type: token.type,
						src: isRelativeUrl(token.src) ? baseUrl + token.src : token.src
					});
				}
			});
		}
